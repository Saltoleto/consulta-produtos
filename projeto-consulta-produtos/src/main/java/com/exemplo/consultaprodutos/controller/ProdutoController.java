...
public class ProdutoController {
    ...
    /**
     * ENDPOINT PRINCIPAL: Busca usuários associados a um produto.
     * GET /api/produtos/{id}/usuarios
     */
    @GetMapping("/{id}/usuarios")
    public ResponseEntity<List<UsuarioResponseDTO>> buscarUsuariosPorProduto(@PathVariable Long id) {
        List<UsuarioResponseDTO> response = produtoService.buscarUsuariosPorProduto(id);
        return ResponseEntity.ok(response);
    }

    /**
     * ENDPOINT PRINCIPAL: Busca usuários associados a um produto com paginação.
     * GET /api/produtos/{id}/usuarios/paginado
     */
    @GetMapping("/{id}/usuarios/paginado")
    public ResponseEntity<Page<UsuarioResponseDTO>> buscarUsuariosPorProdutoComPaginacao(
            @PathVariable Long id,
            @PageableDefault(size = 20) Pageable pageable) {
        
        Page<UsuarioResponseDTO> response = produtoService.buscarUsuariosPorProdutoComPaginacao(id, pageable);
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<List<ProdutoResponseDTO>> listarProdutosComUsuarios() {
        List<ProdutoResponseDTO> response = produtoService.listarProdutosComUsuarios();
        return ResponseEntity.ok(response);
    }
